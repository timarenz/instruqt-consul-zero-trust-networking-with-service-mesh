...
echo
ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -i ssh.key consul-admin@postgres-server sudo netstat -tulpn
echo
...
echo
ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -i ssh.key consul-admin@postgres-server sudo tcpdump -nX -i ens4 port 5432 or port 21000
echo
echo "All done."
echo