...
echo
ssh -i ssh.key consul-admin@postgres-server sudo netstat -tulpn
echo
...
echo
ssh -i ssh.key consul-admin@postgres-server sudo tcpdump -nX -i ens4 port 5432 or port 21000
echo
echo "All done."
echo