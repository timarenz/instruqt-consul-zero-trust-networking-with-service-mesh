...
echo
ssh -i ssh.key consul-admin@postgres-server sudo diff -y /etc/consul.d/postgres-service.json /etc/consul.d/postgres-service.json.example
echo
echo "Next we will enable service mesh for the postgres server by replacing the service configuration file."
read -n 1 -r -s -p "Press any key to continue..."
echo
ssh -i ssh.key consul-admin@postgres-server sudo cp /etc/consul.d/postgres-service.json.example /etc/consul.d/postgres-service.json
echo
...
echo "Next we will enable service mesh for the postgres server by replacing the service configuration file."
ssh -i ssh.key consul-admin@postgres-server "systemctl restart consul && systemctl enable envoy && systemctl restart envoy"
echo
echo "All done."
echo